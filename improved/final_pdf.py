...
def place_wrapped_text_and_images(text_arr, image_list, base_path,
    ...
    for t in text_arr:
        ...
        test_text = strip_html_tags(t).strip()
    ...
    if image_list and in_template:
        # Prepare first image for integration with text
    ...
    if cleaned_text_arr:
        ...
        if image_info and in_template and frame:
            try:
                ...
                # When image takes up width, text needs more vertical space
                # Rough estimate: (image_width / frame_width) * image_height
                width_ratio = img_width / frame_w
                displacement_height = img_height * width_ratio * 1.2  # 1.2x safety factor

                # Extend the text frame by the displacement height
                # Ensure frame is at least as tall as the image itself
                current_w, current_h = scribus.getSize(frame)
                new_height = max(current_h + displacement_height, img_height)
                ...
                # Use the maximum of image height or extended frame height
                # This ensures y_offset is past BOTH text and image
                final_height = max(new_height, img_height)
                y_offset = actual_frame_y + final_height + 6  # Minimal spacing after image (reduced from BLOCK_SPACING * 2)
            except:
                pass

        # Then place image on top if available - use ACTUAL FRAME position
        # Only place image if this is template text
        if image_info and in_template:
            box_padding = 2
            img_width = image_info['width']
            img_height = image_info['height']
            ...
            # Position at left margin at ACTUAL frame Y position (not saved_y_offset)
            # This ensures images are placed consistently at the top of the text frame,
            # even if the frame moved due to page breaks or repositioning
            img_x = MARGINS[0]
            # Use actual frame position instead of saved_y_offset for consistent placement
            # ALWAYS place image at the top of the text frame for consistency
            if frame:
            ...
            # DO NOT adjust current_y based on frame height - we want all images at the same relative position (top)
...
def process_template(tmpl, base_path, is_continuation=False, next_is_continuation=False):
    ...
    tid = tmpl.get("id","0")
    template_name = tmpl.get("text", [""])[0][:50] if tmpl.get("text") else f"Template {tid}"
